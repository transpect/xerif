...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
  ...
  <xsl:template match="para[matches(@role, $figure-caption-role-regex)]" mode="figures">
    <title>
      <xsl:apply-templates mode="hub:split-at-tab"/>
    </title>
  </xsl:template>
...
<!-- group meta infos for same structure as in IDML-->
 <xsl:template match="*[*[starts-with(@role, 'tsmeta') and not(matches(@role, 'tsmeta(keyword|alternativeheadline)'))]]" mode="hub:meta-infos-to-sidebar">
    <xsl:copy copy-namespaces="no">
      <xsl:apply-templates select="@*" mode="#current"/>
      <xsl:for-each-group select="node()" group-adjacent="exists(self::para[starts-with(@role, 'tsmeta') and not(matches(@role, 'tsmeta(keyword|alternativeheadline)'))])">
        <xsl:choose>
          <xsl:when test="current-grouping-key()">
 ...
 <!-- sort metadata in chapter  -->
  ...
  <xsl:template match="section[@role = ('abstract', 'keywords', 'alternative-title')][not(title)]/para[matches(@role, '[a-z]{1,3}(abstract|metakeywords?|metaalternativeheadline)')][1]/node()[1][self::phrase]" mode="hub:process-meta-sidebar" priority="2">
  ...
  <xsl:template match="section[@role = ('abstract', 'keywords', 'alternative-title')][not(title)]/para[matches(@role, '[a-z]{1,3}(abstract|metakeywords?|metaalternativeheadline)')][1][node()[1][self::phrase]]/node()[2][self::text()]" mode="hub:process-meta-sidebar" priority="2">
    <xsl:value-of select="replace(., '^[\p{Zs}]+', '')"/>
  </xsl:template>
  ...
  <xsl:template match="section[para[@role[matches(., '[a-z]{1,3}abstracts?$')]]]
                              [every $i in * 
                               satisfies (   $i/self::title 
                                          or $i/self::para[@role[matches(., '[a-z]{1,3}abstracts?$')]])
  ...
  <xsl:template match="*[para[matches(@role, '[a-z]{1,3}(abstracts?|metakeywords?|alternativeheadline)')]
                             [not(parent::abstract)]]" mode="hub:repair-hierarchy" priority="2">
    <xsl:copy copy-namespaces="no">
      <xsl:apply-templates select="@*" mode="#current"/>
      <xsl:for-each-group select="*" group-adjacent="exists(.[self::para][@role[matches(., '[a-z]{1,3}(abstracts?|metakeywords?|alternativeheadline)(heading)?')]])">
        <xsl:variable name="all-abstract-and-keyword-para" as="element(*)*" select="current-group()"/>
        <xsl:for-each-group select="current-group()" group-starting-with="para[@role[matches(., '[a-z]{1,3}(abstracts?|metakeywords?|alternativeheadline)heading')] 
                                                                            or 
                                                                            (@role[matches(., '[a-z]{1,3}(abstracts?|metakeywords?|alternativeheadline)($|_-_)')] 
                                                                              and not($all-abstract-and-keyword-para[@role[matches(., '[a-z]{1,3}(abstracts?|metakeywords?|alternativeheadline)heading')]]) 
  ...
  <xsl:template match="para[matches(@role, '[a-z]{1,3}(abstracts?|metakeywords?|alternativeheadline)heading')]" mode="hub:repair-hierarchy" priority="2">
    <title>
      <xsl:apply-templates select="@*, node()" mode="#current"/>
    </title>
  </xsl:template>
